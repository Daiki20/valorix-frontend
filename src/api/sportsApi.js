...
${awayRoster ? `${match.away}: ${awayRoster}` : `${match.away}: состав неизвестен`}`
    : `СОСТАВЫ: данные не получены — НЕ придумывай имена игроков и их характеристики. Напиши что составы неизвестны.`
...
${ctx.awayResults ? `${match.away}: ${ctx.awayResults}` : ''}`.trim()
  ...
  const dataQuality = hasRealRoster && (ctx.homeResults || ctx.awayResults)
    ? '✓ Реальные данные получены — используй их как основу анализа, они приоритетнее твоих знаний.'
    : '⚠ Реальных данных из API нет — ЗАПРЕЩЕНО выдумывать имена игроков, их роли, статистику и личные характеристики. Анализируй только по общеизвестным фактам об организации. Обязательно укажи в dataWarning что конкретных данных нет.'
...
ЕСЛИ в блоке данных выше есть реальные составы и результаты — используй только их, это главный приоритет.
ЕСЛИ данных нет — пиши только то, что ТОЧНО известно об организации (история, регион, стиль). НЕ придумывай имена игроков, статистику, характеристики конкретных людей.
dataWarning ОБЯЗАТЕЛЕН если нет ростера или результатов матчей.

ТРЕБОВАНИЯ к extraBets: ОБЯЗАТЕЛЬНО 3-4 ставки. Минимальный коэф ≥ 1.75, целевой диапазон 1.80 — 3.00. Ищи форы, точный счёт карт, конкретные исходы. Пустой массив НЕДОПУСТИМ.
