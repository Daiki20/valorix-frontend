...
const STEPS = [
  {
    icon: <Zap size={32} color={ACCENT} fill={ACCENT} />,
    title: 'Добро пожаловать в Valorix AI! 🎉',
    desc: 'Вам начислено 50 монет. Это ваш стартовый баланс для первых анализов. Расскажем как всё работает.',
    hint: null,
  },
  {
    icon: <Search size={32} color={ACCENT} />,
    title: 'Выбирайте матч из списка',
    desc: 'Перейдите в "Анализ матча" — там загрузятся предстоящие матчи из топ-лиг. Выберите любой.',
    hint: '28 монет за один анализ',
  },
  {
    icon: <Upload size={32} color={A2} />,
  ...
  {
    icon: <Zap size={32} color="#22c55e" fill="#22c55e" />,
  ...
  return (
    <>
      <div onClick={finish} style={{
        position: 'fixed', inset: 0, background: 'rgba(3,11,24,0.8)',
      ...
      <div style={{
        ...
        <div className="card" style={{
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 28 }}>
            {STEPS.map((_, i) => (
              <div key={i} style={{
                ...
                background: i <= step
                  ? `linear-gradient(90deg, ${ACCENT}, ${A2})`
                  : 'rgba(0,180,255,0.1)',
          ...
          <div style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            marginBottom: 24, boxShadow: '0 4px 20px rgba(0,207,255,0.15)',
          }}>
            {current.icon}
          </div>

          <h2 style={{ fontSize: 20, fontWeight: 800, color: TEXT, marginBottom: 12, lineHeight: 1.3 }}>
            {current.title}
          </h2>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {step > 0 && (
              <button onClick={() => setStep(s => s - 1)} style={{
                ...
                borderRadius: 10, background: 'rgba(0,15,40,0.5)', cursor: 'pointer',
            ...
            <button
              ...
              style={{
                ...
                fontSize: 14, fontWeight: 700, color: '#030b18',
